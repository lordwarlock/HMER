...
class SymbolRecognition(object):
	...
	def image_deformation(self,image):
		...
		random_rot_angl = np.random.random() * np.pi/3 - np.pi/6 - random_shear_angl
		random_x_scale = np.random.random() * .6 + .7
		random_y_scale = np.random.random() * .6 + .7
		random_x_trans = np.random.random() * image.shape[0] / 2 - image.shape[0] / 4
		random_y_trans = np.random.random() * image.shape[1] / 2 - image.shape[1] / 4
		dx = image.shape[0]/2. \
	...
	def inference(self,target_num = 10):
		...
		if self.trainflag:
			...
			#self.y_ = tf.placeholder(tf.float32,[None,target_num])
		"""
		W_prep = self.weight_variable([5, 5, 1, 8])
		tmp_prep,_,_ = self.batch_norm_layer(self.conv2d(self.x, W_prep,padding=padding))
		h_prep = tf.nn.relu(tmp_prep)
		h_prep_pool = self.max_pool_2x2(h_prep)
		"""
		W_conv1 = self.weight_variable([1, 1, 1, 8])
		...
		W_conv3 = self.weight_variable([1, 1, 8, 32])	
		tmp_3,_,_ = self.batch_norm_layer(self.conv2d(h_conv1, W_conv2,padding=padding))
		h_conv3 = tf.nn.relu(self.x)
